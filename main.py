...
st.title("Objective Risk Identifier")

col1, col2 = st.columns(2)

with col1:
    objective = st.text_area("Please provide objective details.", value="", height=200)
...
if clicked:
    if not objective or not num_risks or not risk_category:
        st.warning("Please fill in all the information.")

    else:
        with st.spinner("Please wait..."):
            response = prompting.generate_risks(objective, num_risks, risk_category)
